...
uint8_t gc_minmax_line(char *line, float *x, float *y, char NoG0) 
{
  char letter;
  float value;
  int int_value;
    ...
    switch(letter) {
      case 'G':
        ...
        switch(int_value) {
          ...
          case 0: if (NoG0) return 0;	//	Nechcem zobrazit kam az zajde posuv
						
					case 1: case 2: case 3: case 80: group_number = MODAL_GROUP_1; break;
  ...
  while(next_statement(&letter, &value, line, &char_counter)) {
    switch(letter) {
      ...
      case 'X': *x = to_millimeters(value); bit_true(axis_words,bit(X_AXIS)); ret = 1;break;
      case 'Y': *y = to_millimeters(value); bit_true(axis_words,bit(Y_AXIS)); ret = 1;break;
    ...
    switch(letter) {
      ...
      case 'M':
        ...
        switch(int_value) {
          ...
          case 2: gc.program_flow = PROGRAM_FLOW_COMPLETED_STOP; break; // Program end 
          case 30: gc.program_flow = PROGRAM_FLOW_COMPLETED; break; // Program reset 
    ...
    switch(letter) {
      ...
      case 'F': 
        ...
        if (gc.inverse_feed_rate_mode) {
          inverse_feed_rate = 2.5 * to_millimeters(value); // seconds per motion for this motion only
        } else {          
          gc.feed_rate = to_millimeters(value) / 2.5; // millimeters per minute
        }
        break;
...
static int next_statement(char *letter, float *float_ptr, char *line, uint8_t *char_counter) 
{
  while (line[*char_counter] == 32) 	//	Toto som doplnil ja aby mohli byt v prikaze aj medzery
		(*char_counter)++;
  if (line[*char_counter] == 0) 
    return(0); // No more statements
  if (line[*char_counter] == 0x0A) //	Toto som doplnil ja aby bralo aj koniec riadku
    return(0); // No more statements
