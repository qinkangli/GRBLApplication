...
//    step_pulse_time = -(((settings.pulse_microseconds-2)*TICKS_PER_MICROSECOND) >> 3);		//snil
...
*/				
		//	printf("out = %x\r\n",STEPPING_PORT);	
//			_delay_us(100);	//	Driver pozaduje 1.5uS //settings.pulse_microseconds
				
			OutputControl((STEPPING_PORT & ~STEP_MASK) | 0x00);	//	na piny poslem pohyb
		//	printf("out = %x\r\n",STEPPING_PORT);	
				
			// Enable step pulse reset timer so that The Stepper Port Reset Interrupt can reset the signal after
			// exactly settings.pulse_microseconds microseconds, independent of the main Timer1 prescaler.
		//	_delay_us(30);	//	Driver pozaduje 1.5uS
...
//    if((GPIOE->IDR & (1 << 4)) == 0)    //  Stlaceny emergeny vstup
//    {
