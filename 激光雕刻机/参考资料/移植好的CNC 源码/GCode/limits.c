...
//aa#include <util/delay.h>
//aa#include <avr/io.h>
//aa#include <avr/interrupt.h>
...
int myround(float num)		//四舍五入转整形
{
		int lnum = num;	//	Celociselna cast
		if(num >= 0)
		{
				if((num - lnum) >= 0.5)
					return lnum+1;
...
long int mylround(float num)
{
		long int lnum = num;	//	Celociselna cast
		if(num >= 0)
		{
				if((num - lnum) >= 0.5)
					return lnum+1;
...
static void homing_cycle(uint8_t cycle_mask, int8_t pos_dir, bool invert_pin, float homing_rate) 
{
  ...
  if (cycle_mask & (1<<X_AXIS)) { 
    dist++;
    steps[X_AXIS] = mylround(settings.steps_per_mm[X_AXIS]); 
  }
  if (cycle_mask & (1<<Y_AXIS)) { 
    dist++;
    steps[Y_AXIS] = mylround(settings.steps_per_mm[Y_AXIS]); 
  }
  if (cycle_mask & (1<<Z_AXIS)) {
    dist++;
    steps[Z_AXIS] = mylround(settings.steps_per_mm[Z_AXIS]);
  }
  ...
  dt_min = mylround(1000000*60/(ds*homing_rate)); // Cruising (usec/step)
...
//    STEPPING_PORT = out_bits0;
		...
		OutputControl((STEPPING_PORT & ~STEP_MASK) | (out_bits & STEP_MASK));
    _delay_us(settings.pulse_microseconds);
    OutputControl(out_bits0);
...
{ 
  ...
  homing_cycle(HOMING_SEARCH_CYCLE_0, true, false, settings.homing_seek_rate);  // Search cycle 0 z
...
//  #endif
  _delay_ms(settings.homing_debounce_delay); // Delay to debounce signal
  ...
  while (n_cycle--) {
    ...
    _delay_ms(settings.homing_debounce_delay);
